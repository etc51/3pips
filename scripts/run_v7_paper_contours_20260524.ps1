...
function Start-Portfolio {
    ...
    $args = @(
        ...
        "--seed-minutes", "240",
        "--orderbook-depth", "10",
        "--profiles", "reports\futures_scalp_profiles_v7_paper_20260524.csv",
        ...
        "--fallback-poll-sec", "2",
        "--snapshot-sec", "10",
        "--log", "reports\paper_runs\v7_live_20260524\${Name}_multi_futures_paper_trades.csv",
        "--snapshot-log", "reports\paper_runs\v7_live_20260524\${Name}_live_orderbook_snapshots.csv",
        "--open-positions-log", "reports\paper_runs\v7_live_20260524\${Name}_paper_open_positions.json",
        "--instrument-specs-log", "reports\paper_runs\v7_live_20260524\${Name}_instrument_specs.csv",
        "--shadow-log", "reports\paper_runs\v7_live_20260524\${Name}_shadow_exit_models.csv"
    )
...
function Write-PortfolioConfig {
    $config = [ordered]@{
        run_name = "v7_live_20260524"
        capital_per_contour = 800000
        profiles_csv = "reports/futures_scalp_profiles_v7_paper_20260524.csv"
        portfolios = [ordered]@{
...
$script:RunDir = Join-Path $script:ProjectRoot "reports\paper_runs\v7_live_20260524"
New-Item -ItemType Directory -Force -Path $script:RuntimeDir | Out-Null
New-Item -ItemType Directory -Force -Path $script:RunDir | Out-Null
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$script:LogPath = Join-Path $script:RuntimeDir "v7_paper_contours_20260524_${Mode}_${timestamp}.log"
...
if ($Mode -eq "dashboard" -or $Mode -eq "all") {
    ...
    $dashArgs = @("src\paper_dashboard.py", "--port", "$DashboardPort", "--dir", "reports\paper_runs\v7_live_20260524")
    $dashOut = Join-Path $script:RuntimeDir "v7_paper_dashboard_20260524.stdout.log"
    $dashErr = Join-Path $script:RuntimeDir "v7_paper_dashboard_20260524.stderr.log"
    ...
    $dash.Id | Set-Content -LiteralPath (Join-Path $script:RuntimeDir "v7_paper_dashboard_20260524.pid") -Encoding ASCII
    Write-RunLog "DashboardPid=$($dash.Id)"
    Write-RunLog "DashboardUrl=http://127.0.0.1:$DashboardPort/"
}

if ($Mode -eq "main" -or $Mode -eq "all") {
    Stop-MatchingPython "multi_futures_paper.py*scalp_once_20260524"
    Stop-MatchingPython "multi_futures_paper.py*v7_live_20260524"
