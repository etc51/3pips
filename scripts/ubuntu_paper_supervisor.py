...
class Supervisor:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.supervisor_pid_path = self.runtime_dir / "v7_paper_supervisor_20260525.pid"
    ...
    def bot_args(self, name: str, secids: list[str]) -> list[str]:
        ...
        args = [
            ...
            *secids,
            ...
            "--startup-status-log", f"reports/paper_runs/{RUN_NAME}/{name}_startup_status.csv",
            "--shadow-log", f"reports/paper_runs/{RUN_NAME}/{name}_shadow_exit_models.csv",
            "--entry-audit-log", f"reports/paper_runs/{RUN_NAME}/{name}_entry_audit.csv",
            "--enable-gpt-shadow",
    ...
    def check_bot(self, name: str) -> None:
        pid = self.read_pid(self.pid_path(name))
        if not self.process_alive(pid):
        ...
        if health_age > self.stale_sec and age >= self.startup_grace_sec:
            ...
            return
        snapshot_path = self.run_dir / f"{name}_live_orderbook_snapshots.csv"
    ...
    def check_dashboard(self) -> None:
        ...
        if not self.dashboard_http_ok():
            age = self.process_age_sec("dashboard")
            if age < 20:
                self.log(f"wait dashboard pid={pid} startup_age_sec={age}")
                return
    ...
    def write_portfolio_config(self) -> None:
        ...
        (self.run_dir / "portfolio_config.json").write_text(
    ...
    def stop_all_children(self) -> None:
        self.stopping = True
        for name in [*PORTFOLIOS.keys(), "dashboard"]:
            path = self.dashboard_pid_path() if name == "dashboard" else self.pid_path(name)
            self.stop_pid(self.read_pid(path), "supervisor_shutdown")
...
def main() -> None:
    ...
    def handle_stop(signum: int, frame: object) -> None:
        supervisor.log(f"signal_received signum={signum}")
        supervisor.stop_all_children()
        raise SystemExit(0)
