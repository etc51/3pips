...
OUT_DIR = REPORTS / "cloud_new_candidates_stress_package"

CURRENT_RUNNING = {"LKM6", "BMM6", "BRM6", "S1M6", "GDM6"}
NEW_STRONG_MOEX = ["GLM6", "CHM6", "TTM6", "RNM6", "BTM6"]
NEW_WATCH_MOEX = ["SiU6", "CEM6", "GNU6", "FSM6", "S1U6"]
NEO_PERP = ["AMZNperpA", "TSLAperpA", "COINperpA", "AMDperpA"]
EXTRA_HIGH_RISK_KEEP = ["PDU6", "PTU6"]
TICKERS = NEW_STRONG_MOEX + NEW_WATCH_MOEX + NEO_PERP + EXTRA_HIGH_RISK_KEEP
...
def prompt_text() -> str:
    return """You are given a compact ZIP with selected candidate futures from a previous all-universe MOEX/T-Bank futures search.
...
def main() -> None:
    ...
    stress = pd.read_csv(Path.home() / "Downloads" / "all_futures_stress_summary.csv")
    specs = pd.read_csv(REPORTS / "cloud_all_futures_grid_package" / "instrument_specs.csv")
    selected = []
    counts = []
    for ticker in TICKERS:
        path = DATA_DIR / f"{ticker}_1m.csv"
        if not path.exists():
            print(f"missing candles: {ticker}")
            continue
        df = read_csv(path)
        selected.append(df)
        counts.append({"ticker": ticker, "rows": len(df)})
    panel = pd.concat(selected, ignore_index=True, sort=False).sort_values(["secid", "time"])
    seed = stress[stress["ticker"].isin(TICKERS)].copy()
    spec_sel = specs[specs["ticker"].isin(TICKERS)].copy()
    counts_df = pd.DataFrame(counts)
    groups = {
        "current_running_excluded": sorted(CURRENT_RUNNING),
        "new_strong_moex": NEW_STRONG_MOEX,
        "new_watch_moex": NEW_WATCH_MOEX,
        "neo_perp_separate": NEO_PERP,
        "extra_high_risk_keep": EXTRA_HIGH_RISK_KEEP,
    }

    panel.to_csv(OUT_DIR / "selected_new_candidate_1m.csv", index=False)
    seed.to_csv(OUT_DIR / "candidate_seed_profiles.csv", index=False)
    spec_sel.to_csv(OUT_DIR / "instrument_specs.csv", index=False)
    counts_df.to_csv(OUT_DIR / "row_counts.csv", index=False)
    (OUT_DIR / "candidate_groups.json").write_text(json.dumps(groups, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "PROMPT_FOR_CHATGPT_NEW_CANDIDATES_STRESS.md").write_text(prompt_text(), encoding="utf-8")
    metadata = {
        "tickers": TICKERS,
        "rows": int(len(panel)),
        "ticker_count": int(panel["secid"].nunique()),
        "current_running_excluded": sorted(CURRENT_RUNNING),
    }
    (OUT_DIR / "metadata.json").write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")

    zip_path = REPORTS / "cloud_new_candidates_stress_package.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        for path in [
            OUT_DIR / "selected_new_candidate_1m.csv",
            OUT_DIR / "candidate_seed_profiles.csv",
            OUT_DIR / "instrument_specs.csv",
            OUT_DIR / "row_counts.csv",
            OUT_DIR / "candidate_groups.json",
            OUT_DIR / "metadata.json",
            OUT_DIR / "PROMPT_FOR_CHATGPT_NEW_CANDIDATES_STRESS.md",
        ]:
