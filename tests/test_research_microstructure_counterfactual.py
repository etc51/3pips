...
class ResearchMicrostructureCounterfactualTest(unittest.TestCase):
    def test_build_and_persist_dedupes_entry_rows_and_prefers_actionable_all_sample_candidate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(summary["unique_entries_day"], 5)
            self.assertEqual(summary["source_entry_shadow_rows_all"], len(rows))
            self.assertEqual(summary["source_entry_shadow_rows_day"], 10)
            self.assertIn("Review trade-level counterfactual candidate", summary["next_action"])
            ...
            self.assertEqual(latest_summary["source_entry_shadow_rows_all"], len(rows))
            self.assertIn("Review trade-level counterfactual candidate", latest_summary["next_action"])
